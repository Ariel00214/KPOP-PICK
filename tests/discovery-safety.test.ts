...
describe("discovery safety",()=>{
 ...
 it("accepts one traceable official source for review but never auto-publishes",async()=>{
  ...
  const result=await new ReleaseDiscoveryService(providers,repo).discover("YESUNG",new Date("2026-09-19"));expect(result.candidates[0].status).toBe("NEEDS_REVIEW");
 });
